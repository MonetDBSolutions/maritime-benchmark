...
SELECT q1.mmsi as mmsi1, q2.mmsi as mmsi2, st_distance(q1.geom::Geography,q2.trajectory::Geography,false) as distance, q1.ts
FROM ais_data.dynamic_sar as q1
INNER JOIN trajectories_10k as q2
ON TRUE
ORDER BY q1.mmsi, q2.mmsi, q1.ts;

COPY comparison.distance_sar_trajectories_10k TO '/Users/bernardo/Monet/VesselAI/Prototype/Comparisons/Distance/distance_sar_trajectories_10k_postgres.csv' DELIMITER ',';

SELECT q1.mmsi as mmsi1, q2.mmsi as mmsi2, q1.ts, st_distance(q1.geom::Geography,q2.trajectory::Geography,false) as distance
FROM ais_data.dynamic_sar as q1
INNER JOIN trajectories_10k as q2
...
SELECT q1.mmsi as mmsi, q2.gid as gid, st_distance(q1.geom::Geography,st_transform(q2.geom,4326)::Geography,false) as distance
FROM ais_data.dynamic_sar as q1
INNER JOIN geographic_features.europe_coastline as q2
ON q1.mmsi <= 2051
ORDER BY q1.mmsi, q2.gid;

-- Covers between fishing_areas and dynamic_sar
CREATE TABLE comparison.covers_fishing_sar AS
SELECT q1.mmsi as mmsi, q2.gid as gid, st_covers(q1.geom::Geography,q2.geom::Geography) as distance
FROM ais_data.dynamic_sar as q1
INNER JOIN geographic_features.fishing_areas_eu as q2
ON TRUE
